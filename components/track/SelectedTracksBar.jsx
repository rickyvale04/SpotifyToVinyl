...
const SelectedTracksBar = ({ selectedTracks, onClearSelection, onSearchVinyl }) => {
  ...
  return (
    <div className={`fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-300 ${
      ...
      <div className="bg-white border border-gray-300 shadow-2xl px-6 py-4 rounded-lg flex items-center space-x-6 min-w-96 max-w-2xl">
        ...
        <div className="flex-1 flex items-center space-x-2 overflow-hidden">
          <div className="flex -space-x-2">
            {selectedTracks.slice(0, 3).map((track, index) => (
              <div key={track.id} className="w-8 h-8 bg-gray-200 border-2 border-white rounded overflow-hidden">
            ...
            {selectedTracks.length > 3 && (
              <div className="w-8 h-8 bg-gray-100 border-2 border-white rounded flex items-center justify-center">
                <span className="text-xs font-medium text-gray-600">+{selectedTracks.length - 3}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="bg-black text-white px-6 py-3 rounded-full text-sm font-medium hover:bg-gray-800 transition-colors flex items-center space-x-2"
          >
